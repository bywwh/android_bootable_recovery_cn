...
int nandroid_backup(const char* backup_path)
{
    ...
    if (vol == NULL || 0 != stat(vol->device, &s))
    {
        ui_print("未找到 sd-ext。跳过对 sd-ext 的备份.\n");
    }
    else
    {
        if (0 != ensure_path_mounted("/sd-ext"))
            ui_print("无法挂载 sd-ext。此设备可能不支持对 sd-ext 进行备份，跳过对sd-ext的备份.\n");
