...
int show_install_update_menu() {
    ...
    static const char* headers[] = { "确认安装SD上的zip文件", "", NULL };
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "adb sideload模式";
...
void show_choose_zip_menu(const char *mount_point) {
    ...
    static const char* headers[] = { "选择一个zip文件安装", "", NULL };
...
void show_mount_usb_storage_menu() {
    ...
    static const char* headers[] = { "U盘模式",
                                     "离开当前菜单即会",
                                     "退出U盘模式.",
                                     "",
                                     NULL
...
int confirm_selection(const char* title, const char* confirm) {
    ...
    const char* confirm_headers[] = { title, "  注意:操作后将无法恢复.", "", NULL };
    ...
    if (many_confirm) {
        char* items[] = { confirm_str, // Yes, [0]
                        "取消 - 返回",
                          NULL };
        int chosen_item = get_menu_selection(confirm_headers, items, 0, 0);
        ret = (chosen_item == 0);
    } else {
        char* items[] = { confirm_str, // Yes, [1]
                            "取消 - 返回",
                          NULL };
        int chosen_item = get_menu_selection(confirm_headers, items, 0, 0);
        ret = (chosen_item == 0);
...
int format_device(const char *device, const char *path, const char *fs_type) {
    ...
    if (strcmp(fs_type, "rfs") == 0) {
        if (ensure_path_unmounted(path) != 0) {
            LOGE("format_volume 不能卸载 \"%s\"\n", v->mount_point);
            return -1;
        }
        if (0 != format_rfs_device(device, path)) {
            LOGE("format_volume: format_rfs_device 不能格式化 %s\n", device);
            return -1;
        }
        return 0;
    }
    ...
    if (strcmp(fs_type, "yaffs2") == 0 || strcmp(fs_type, "mtd") == 0) {
        ...
        if (partition == NULL) {
            LOGE("format_volume: 无MTD分区 \"%s\"\n", device);
            return -1;
        }

        MtdWriteContext *write = mtd_write_partition(partition);
        if (write == NULL) {
            LOGW("format_volume: 无法打开MTD设备 \"%s\"\n", device);
            return -1;
        } else if (mtd_erase_blocks(write, -1) == (off_t) - 1) {
            LOGW("format_volume: 无法擦除MTD设备 \"%s\"\n", device);
            mtd_write_close(write);
            return -1;
        } else if (mtd_write_close(write)) {
            LOGW("format_volume: 无法关闭MTD设备 \"%s\"\n", device);
            return -1;
        }
        return 0;
    }
...
int show_partition_menu() {
    static const char* headers[] = { "挂载及U盘模式", "", NULL };
...
static void add_nandroid_options_for_volume(char** menu, char* path, int offset) {
    ...
    sprintf(buf, "从%s中选择备份文件进行还原", path);
    menu[offset + 1] = strdup(buf);

    sprintf(buf, "从%s删除备份文件", path);
    menu[offset + 2] = strdup(buf);

    sprintf(buf, "从%s中选择备份文件进行高级还原", path);
...
void format_sdcard(const char* volume) {
    ...
    if (chosen_item < 0) // REFRESH or GO_BACK
        return;
    if (!confirm_selection("确认需要格式化?", "是的 - 格式化此设备"))
        return;
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 3: {
                ...
                if (confirm_selection("确定 清空?", "是的 - 清空 Dalvik 缓存")) {
                    __system("rm -r /data/dalvik-cache");
                    __system("rm -r /cache/dalvik-cache");
                    __system("rm -r /sd-ext/dalvik-cache");
                    ui_print("Dalvik 缓存已清空.\n");
                }
...
int verify_root_and_recovery() {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("root访问权限设置不当.是否修正？", "是的 - 修正root(/system/xbin/su)")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("未添加root访问所需文件.是否添加？", "是的- 确认添加 (/system/xbin/su)")) {
