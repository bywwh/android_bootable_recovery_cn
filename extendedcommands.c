...
#endif 
    if (one_confirm) {
        char* items[] = { confirm, //" Yes -- wipe partition",   // [1]
                        "取消 - 返回",
                        NULL };
...
int format_unknown_device(const char *device, const char* path, const char *fs_type)
{
    ...
    if (0 == strcmp(path, "/sd-ext"))
    {
        ...
        {
            ui_print("未找到 app2sd 分区。跳过对 /sd-ext 的格式 /sd-ext.\n");
            return 0;
        }
    }
...
void show_partition_menu()
{
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
          options[mountable_volumes + formatable_volumes] = "开启 U 盘模式";
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择一个要还原的镜像",
                                "",
                                "首先选择一个要还原的镜",
                                "像。接下来的菜单会有更",
                                "多的选项以供选择。",
...
{
    static char* headers[] = {  "高级功能菜单",
                                "",
                                NULL
    };

    static char* list[] = { "重启 recovery",
                            "清空Dalvik 缓存",
                            "清空电池信息",
                            "错误报告",
                            "按键测试",
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 1:
                ...
                if (confirm_selection( "确定 清空?", "是的 - 清空Dalvik 缓存")) {
